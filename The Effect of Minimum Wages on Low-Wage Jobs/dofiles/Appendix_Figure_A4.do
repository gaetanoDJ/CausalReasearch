...
gen totalpopulation = population
...
foreach b in   1979   {
	foreach w in 1  {
		...
		use `temptemp', clear
			...
			qui reghdfe logMW $treat if  year>=`b' & cleansample==1 ${weight`w'} , absorb(i.statenum i.quarterdate  postcont postcontf  precont precontf earlycont earlycontf ) cluster(statenum)
			...
			sort time
						...
						twoway (scatter  est time  if time!=., msymbol(D) mcolor(blue) yline(-1, lcolor(gs10) lwidth(vvthin)) xline(-1, lpattern(dash) lcolor(black) )) ///
						(line  est time  , lcolor(blue) lpattern(dash)  lwidth(thick) ) ///
						(rcap low  high time , lcolor(blue) lwidth(thick) ), graphregion(color(white)) ///
						xlabel(-3(1)4, labsize(medsmall))	///
						ylabel(, labsize(medsmall)) legend(off) xtitle("Years relative to the minimum wage change", size(medsmall))  ylabel(-0.2(0.1)0.2) ///
						ytitle("Percentage change in MW", height(6))   						
			
			gr export "${figures}FigureA4.pdf", replace
