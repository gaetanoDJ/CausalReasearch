...
clear *
*This data is created in Appendix_Figure_D1
...
gen T=quarterdate>=treatdate-12 & quarterdate<treatdate-8
...
preserve
drop if statenum==11
drop if quarterdate<85
count 
local tot_event = r(N)
cap drop _event
egen _event = rank(estimate_`cat') , unique
...
twoway rbar low_estimate_`cat'2 hi_estimate_`cat'2 _event , horizontal color(gs12)  ///
...
gr export "${figures}FigureD2_a.pdf", replace
restore
}
*
