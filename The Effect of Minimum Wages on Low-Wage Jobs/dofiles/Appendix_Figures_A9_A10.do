...
local controls ""
			...
			binscatter emp_estimate MWtomed if statenum!=11 ///
				...
				gr export "${figures}FigureA9_a.pdf", replace
			...
			di "`graphcommanda'"
				...
				gr export "${figures}FigureA9_b.pdf", replace

	

twoway  (scatter   emp_estimate    MWtomed   if   statenum!=11 [aw=avetotalpopulation_above]   , ///
 msymbol(Oh) mcolor(green*0.50) xlabel(, format(%03.2f) ) )  ///
(  scatter   emp_estimate MWtomed   if   statenum==11 [aw=avetotalpopulation_above]   , ///
  msymbol(Oh) mcolor(red*0.80)   )  ///
(lfit emp_estimate MWtomed if   statenum!=11 [aw=avetotalpopulation_above] , lpattern(straight) lcolor(green) lwidth(thick) ) ///
( lfit emp_estimate MWtomed   [aw=avetotalpopulation_above] ,  lcolor(red)  lpattern(straight)  lwidth(thick)  ), ///
ytitle("Change in employment relative to" "pre-treatment total employment") ///
legend(order(1 "130 state-level events" 2 "D.C. events" 3 "Linear fit of the 130 events" 4 "Linear fit including D.C. events") position(6) cols(2)) graphregion(color(white))
gr export "${figures}FigureA10_a.pdf", replace
	
	
	
twoway  (scatter   emp_estimate    MWtomed   if   statenum!=11    , ///
 msymbol(Oh) mcolor(green*0.50) xlabel(, format(%03.2f) ) )  ///
(  scatter   emp_estimate    MWtomed   if   statenum==11    , ///
  msymbol(Oh) mcolor(red*0.80)   )  ///
(lfit emp_estimate MWtomed if   statenum!=11  , lpattern(straight) lcolor(green) lwidth(thick) ) ///
( lfit emp_estimate MWtomed   ,  lcolor(red)  lpattern(straight)  lwidth(thick)  ), ///
ytitle("Change in employment relative to" "pre-treatment total employment") ///
legend(order(1 "130 state-level events" 2 "D.C. events" 3 "Linear fit of the 130 events" 4 "Linear fit including D.C. events") position(6) cols(2)) graphregion(color(white))
gr export "${figures}FigureA10_b.pdf", replace
