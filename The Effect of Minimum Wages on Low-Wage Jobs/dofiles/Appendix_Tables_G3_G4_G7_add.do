...
/*
global treat   		F12treat F11treat F10treat F9treat F8treat F7treat F6treat F5treat F4treat F3treat F2treat F1treat treat ///
L1treat L2treat L3treat L4treat L5treat L6treat L7treat L8treat L9treat L10treat L11treat L12treat L13treat L14treat L15treat L16treat ///
L17treat L18treat L19treat

global minus (F4treat + F3treat + F2treat + F1treat)/4
...
gen alt_statenum = statenum
replace alt_statenum = statenum + 1000 if year>=1993
...
replace sim_epop4 = L.sim_epop4 * 0 /*+ error_term*/ if year>=1993
...
global a1  i.statenum i.quarterdate  // TWFE
global a2  i.statenum##c.quarterdate i.quarterdate  // State trends
global a321 i.statenum i.quarterdate i.quarterdate#(c.indmajor_* c.occmajor_*)
global a611 i.statenum i.quarterdate i.one#c.pvi_cont
...
foreach b in   1979   {
	foreach w in 1 {
		...
		use `temptemp', clear
				...
				sum teenepop if  year>=`b' & cleansample==1  ${teenweight`w'},meanonly
				local epop = r(mean)
				sum DMW_real if fedincrease!=1 & overallcountgr>0 & year>=`b' & cleansample ==1   ${teenweight`w'}
				local mwc = r(mean)
				sum MW_real if  F.fedincrease!=1 & F.overallcountgr>0 & year>=`b' & cleansample ==1   ${teenweight`w'}
				local mw = r(mean)
				local mwpc = `mwc'/`mw'
				local mwpc2 : di  %04.3f `mwpc'
			...
			*est use "${estimates}Event_agg_`b'_`w'_`s'_epop"
			...
			lincomestadd (($lincomagg) * (1/5))*${C},statname(ests)
			...
			qui reghdfe `regsample'teenepop $treat if  year>=`b' & cleansample==1 ${teenweight`w'} , absorb( ${a`s'} postcont postcontf  precont precontf earlycont earlycontf ) cluster(statenum)
			sum quarterdate if e(sample)==1
			estadd local begquarter = r(min)
			estadd local endquarter = r(max)
			...
			*est use "${estimates}Event_agg_`b'_`w'_`s'_epop"
			...
			lincomestadd (($lincomagg) * (1/5))*${teenC},statname(ests)
			...
			estadd local states "`temp'"
			estadd local outcome "Actual epop"
			eststo teenactual_EBC`s'
			
			*Post 1992

			qui reghdfe epop $treat if  year>=1993 & cleansample==1 ${weight`w'} , absorb( ${a`s'} postcont postcontf  precont precontf earlycont earlycontf ) cluster(statenum)
			sum quarterdate if e(sample)==1
			estadd local begquarter = r(min)
			estadd local endquarter = r(max)
			...
			*est use "${estimates}Event_agg_`b'_`w'_`s'_epop"
			...
			estadd local states "`temp'"
			estadd local outcome "Actual epop"
			eststo post1992_EBC`s'

			
			
			*teen
			qui reghdfe teenepop $treat if  year>=1993 & cleansample==1 ${teenweight`w'} , absorb( ${a`s'} postcont postcontf  precont precontf earlycont earlycontf ) cluster(statenum)
			sum quarterdate if e(sample)==1
			estadd local begquarter = r(min)
			estadd local endquarter = r(max)
