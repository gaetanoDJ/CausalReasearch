...
foreach b in   /* 1995 */ 1979 {
				...
				****************************************************************
				...
				sum wagebins if treat_p0 == 1  & year>=`b' & cleansample ==1  , meanonly
				...
				*di "${rtype`s'} `Y'countpc  ${treatafter} one if  year>=`b'  & cleansample==1 ${if`s'}   , a(${a`s'}  ${control} ${controlbefore} ${controlf} ${window} )  cluster(statenum)"
				*${rtype`s'} `Y'countpc  ${treatafter} one if  year>=`b'  & cleansample==1 ${if`s'}  , a(${a`s'}  ${control} ${controlbefore} ${controlf} ${window} )  cluster(statenum)
				*est save "${estimates}Table1aafterqceww0`limitsample'`limittreat'_`s'_`Y'_`b'_`temp1'_`temp2'_${Tmax}${Tmin}", replace 
...
*Table_create TWFE , group(overall) tablename(Table1_qcew_w0) footnotes(Affected share is `affshoverall')

/*
Table_create TWFE ST STQ DP ST_DP STQ_DP STP, group(overall) tablename(Table1_qcew) footnotes(Affected share is `affshoverall')
Table_create STP15 STP20, group(overall) tablename(Tableadd_qcew) footnotes(Affected share is `affshoverall')
*Table_create TWFE ST STQ DP ST_DP STQ_DP STP, group(overall) tablename(Table_qcew_weight) footnotes(Affected share is `affshoverall')
*/
