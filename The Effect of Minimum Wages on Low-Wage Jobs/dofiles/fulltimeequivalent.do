...
foreach Y in  overallFTE { 
	replace `Y'countpc= `Y'countpc * (multiplier)
	replace `Y'countpcall = `Y'countpcall*(multiplier)
}
*
...
cap drop overallFTEWB*
g overallFTEWBpcFH = (wagebins/100)*(overallFTEcountpc)

cap drop overallFTEWBpcrsumFH
g overallFTEWBpcrsumFH=0
replace overallFTEWBpcrsumFH = overallFTEWBpcFH if wagebins==100
replace overallFTEWBpcrsumFH = overallFTEWBpcrsumFH[_n-1] + overallFTEWBpcFH if wagebins>100
	
cap drop overallFTEcountpcrsum
g overallFTEcountpcrsum=0
replace overallFTEcountpcrsum = overallFTEcountpc if wagebins==100
replace overallFTEcountpcrsum = overallFTEcountpcrsum[_n-1] + overallFTEcountpc if wagebins>100
...
foreach b in   /* 1995 */ 1979 {
				...
				**  ADS
				...
				foreach s in /*0*/ 1 /*4 5 6 10*/ /*11*/ /*0a 0b  4  5 6*/     {		
				...
				*est save "${estimates}Table4abefore`limitsample'`limittreat'_`s'_`Y'_`b'_`temp1'_`temp2'_${Tmax}${Tmin}", replace 
				...
				*est save "${estimates}Table4aafter`limitsample'`limittreat'_`s'_`Y'_`b'_`temp1'_`temp2'_${Tmax}${Tmin}", replace 

				est use "${estimates}Table4aafter`limitsample'`limittreat'_`s'_`Y'_`b'_`temp1'_`temp2'_${Tmax}${Tmin}"
 				local dof = e(df_r)
				local numberobs = e(N)
