...
set more off
	...
	global data "D:\Research Related\dropboxdocuments\Dropbox\Arin RAship\State Panels\"
	global figures "D:\Research Related\dropboxdocuments\Dropbox\Arin RAship\State Panels\"
...
*local alt "keep if wagebins<3500"
*local altname "alt"
...
gen _wagebins = floor((wagebins)/25)*25
drop wagebins
...
*foreach y of numlist 1998 2001 2014{ 
disp "year=`y' & sex=`s'"
...
append using "${data}ML_results_pooled_`vv'_MNWAOR_`altname'.dta"
save "${data}ML_results_pooled_`vv'_MNWAOR_`altname'.dta", replace

}
use "${data}ML_results_pooled_`vv'_MNWAOR_`altname'.dta", clear
...
save "${data}ML_results_pooled_`vv'_MNWAOR_`altname'_fullypool_noshift.dta", replace


use "${data}ML_results_pooled_`vv'_MNWAOR_`altname'_fullypool_noshift.dta", clear
gen p=exp(b1)/(1+exp(b1))
gen rho2=1/(1+exp(2*b2))
gen rho=sqrt(rho2)
gen eta=p+(1-p)*rho
