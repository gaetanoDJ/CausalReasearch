...
replace overallFTEWBpcrsumFH = overallFTEWBpcFH if wagebins==300
replace overallFTEWBpcrsumFH = overallFTEWBpcrsumFH[_n-1] + overallFTEWBpcFH if wagebins>300
	
cap drop overallFTEcountpcrsum
g overallFTEcountpcrsum=0
replace overallFTEcountpcrsum = overallFTEcountpc if wagebins==300
replace overallFTEcountpcrsum = overallFTEcountpcrsum[_n-1] + overallFTEcountpc if wagebins>300
*/
...
foreach b in   /* 1995 */ 1979 {
			...
			*Method 2
			/*
			est use "${estimates}Table4aqcewafter`limitsample'`limittreat'_`s'_`Y'_`b'_`temp1'_`temp2'_${Tmax}${Tmin}"
			abovebelowWBbunch_alt
			abovebelowWBfull_alt
			nlcomestadd ((${aboveWB_full})/${EWB})*(4*`denominator'), statname(WB_E) dof(`dof') 
			eststo `Y'_2
			*/
			*Method 3
			est use "${estimates}Table4aqcewafter`limitsample'`limittreat'_`s'_`Y'_`b'_`temp1'_`temp2'_${Tmax}${Tmin}"
